...
func (m *ProfileORM) ToPB(ctx context.Context) (Profile, error) {
	...
	if m.PreferredTheme != nil {
		to.PreferredTheme = &wrappers.StringValue{Value: *m.PreferredTheme}
	}
...
func DefaultApplyFieldMaskProfile(ctx context.Context, patchee *Profile, patcher *Profile, updateMask *field_mask1.FieldMask, prefix string, db *gorm1.DB) (*Profile, error) {
	...
	var updatedPreferredTheme bool
	for i, f := range updateMask.Paths {
		if f == prefix+"Id" {
			patchee.Id = patcher.Id
			continue
		...
		if !updatedPreferredTheme && strings.HasPrefix(f, prefix+"PreferredTheme.") {
			if patcher.PreferredTheme == nil {
				patchee.PreferredTheme = nil
				continue
			}
			if patchee.PreferredTheme == nil {
				patchee.PreferredTheme = &wrappers.StringValue{}
			}
			childMask := &field_mask1.FieldMask{}
			for j := i; j < len(updateMask.Paths); j++ {
				if trimPath := strings.TrimPrefix(updateMask.Paths[j], prefix+"PreferredTheme."); trimPath != updateMask.Paths[j] {
					childMask.Paths = append(childMask.Paths, trimPath)
				}
			}
			if err := gorm2.MergeWithMask(patcher.PreferredTheme, patchee.PreferredTheme, childMask); err != nil {
				return nil, nil
			}
		}
		if f == prefix+"PreferredTheme" {
			updatedPreferredTheme = true
